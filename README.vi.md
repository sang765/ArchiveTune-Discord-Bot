...
Với video hoặc audio, hãy bỏ `quality` ở lần đầu để mở menu chọn quality. Chọn format trong dropdown rồi bấm **Download**; bot chưa tải cho đến khi bạn xác nhận. Bạn vẫn có thể truyền trực tiếp format ID hoặc `quality:best` nếu muốn. File được đặt tên theo title đã sanitize và type, ví dụ `My_Song_audio.opus`, `My_Video_video.mp4` hoặc `My_Video_thumbnail.webp`; riêng audio từ YouTube Music dùng metadata theo dạng `Tên Bài Hát - Tên Ca Sĩ.opus` và không thêm hậu tố `_audio`. Extension gốc do yt-dlp chọn vẫn được giữ nguyên. File được upload lên [temp.sh](https://temp.sh/) và link có thời hạn tạm thời khoảng ba ngày. Startup script của Pterodactyl tự cài yt-dlp và ffmpeg vào `.tools/media` nếu chưa có; đặt `AUTO_INSTALL_MEDIA_TOOLS=0` nếu muốn tắt cơ chế này.
