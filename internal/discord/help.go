...
func HelpEmbed() *discordgo.MessageEmbed {
	return &discordgo.MessageEmbed{
		...
		Fields: []*discordgo.MessageEmbedField{
			{
				Name:   "General",
				Value:  "`.help` — show this command list\n`.ytd <YouTube URL> type:<video|audio|thumbnail> [quality:<format id>]` — open quality selector, download after confirmation, and return a temporary link",
				Inline: false,
			},
			{
			...
			},
			{
				Name: "Slash commands",
				Value: "`/help` — show this command list\n" +
					"`/ytd <url> <type> [quality]` — open quality selector, download after confirmation, and return a temporary link\n" +
